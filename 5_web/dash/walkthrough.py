    ...
    html.Div([                  # html.Div를 통해 구역을 따로따로 나누고, 스타일을 부여할 수 있음
            ...
            html.Div(
                [
                    ...
                    html.Img(
                        src='https://i.imgur.com/3L3wPOz.png',
        ...
        html.Div([
            html.Div([    # nested div를 하나 더 추가해서 그래프를 각각 따로 관리
                ...
                )], className='five columns'  # 12 컬럼 중 5컬럼을 지정
                ),                           # 이렇게 하면 세로로 분할할 수 있다.
                                             # total 12 :)
            ...
            ], className='seven columns'),  # 12 - 5 = 7
        ], className='row',), # 그래프 부분은 또 하나의 row에 배정한다.

    ], className='ten columns offset-by-one') # 글로벌적으로 좌우에 1columns씩 마진을 기본으로 부여 (offset)
    )
